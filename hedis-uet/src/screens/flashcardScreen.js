...
export default function FlashcardScreen() {
    ...
    useEffect(() => {
        ...
        console.log('card name', flashcard[index].name)
    },[index])
...
return (
    <View style={styles.container}>
        <Animated.View entering={FadeIn.delay(200).duration(1000)} style={{width: wp(100), paddingLeft: 10, paddingTop: 20, backgroundColor:  '#8de06f'}}>
            <TouchableOpacity onPress={()=> navigation.goBack()}>
                <ChevronLeftIcon size={hp(3.5)} strokeWidth={4.5} color="black" />
            </TouchableOpacity>
        </Animated.View>
        <View style={{ justifyContent: 'flex-end',  width: wp(100), height: hp(15), backgroundColor: '#8de06f', padding: 20}}>
                <Text style={{fontSize: 32, fontWeight: 'bold'}}>Flash card</Text>
        </View>
        <View style={styles.mainContext}>
           <View style={styles.box}>
                <TouchableOpacity onPress={() => speak()}>
                    <Image source={flashcard[index].image} style={{width: wp(25), height: hp(20), borderRadius: 10}}/>
                </TouchableOpacity>
           </View>
